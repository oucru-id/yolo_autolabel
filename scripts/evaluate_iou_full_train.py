...
def evaluate_iou_train(model_path: str, dataset_dir: Path, iou_threshold: float = 0.5):
...
def compute_metrics(records: list[dict], class_names: dict) -> dict:
    ...
    for rec in records:
        ...
        if t == "True Positive":  class_counts[c]["tp"] += 1
        elif t == "False Positive": class_counts[c]["fp"] += 1
        elif t == "False Negative": class_counts[c]["fn"] += 1
...
def print_metrics(metrics: dict):
    ov = metrics["overall"]
    print("\n" + "═" * 65)
    print("  OVERALL METRICS  [TRAIN SET]")
    print("═" * 65)
    print(f"  {'Type':<20} {'Count':>8}")
    print(f"  {'-' * 30}")
    print(f"  {'True Positive  (TP)':<20} {ov['tp']:>8}")
    print(f"  {'False Positive (FP)':<20} {ov['fp']:>8}")
    print(f"  {'False Negative (FN)':<20} {ov['fn']:>8}")
    print(f"  {'True Negative  (TN)':<20} {ov['tn']:>8}")
    print(f"  {'-' * 30}")
    print(f"  {'Precision':<20} {ov['precision']:>8.4f}")
    print(f"  {'Recall':<20} {ov['recall']:>8.4f}")
...
def plot_results(records: list[dict], metrics: dict, save_dir: Path):
    ...
    if class_ious:
        fig, ax = plt.subplots(figsize=(max(10, len(class_ious) * 1.5), 6), facecolor="#0d1117")
        ax.set_facecolor("#161b22")
        ax.boxplot(
            [class_ious[c] for c in sorted(class_ious)], patch_artist=True,
            boxprops=dict(facecolor="#58a6ff", color="white", alpha=0.7),
            capprops=dict(color="white"), whiskerprops=dict(color="white"),
        ...
        ax.set_title("IoU Distribution per Class — TRAIN SET (True Positives)",
        ...
        plt.tight_layout()
        plt.savefig(str(save_dir / "train_iou_distribution_per_class.png"),
    ...
    if classes:
        ...
        fig, ax = plt.subplots(figsize=(max(10, len(classes) * 1.5), 6), facecolor="#0d1117")
        ax.set_facecolor("#161b22")
        ...
        ax.set_title("TP / FP / FN per Class — TRAIN SET", color="white",
        ...
        for spine in ax.spines.values(): spine.set_color("#30363d")
        plt.tight_layout()
        plt.savefig(str(save_dir / "train_tp_fp_fn_per_class.png"),
                    dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
        plt.close()
    ...
    if classes:
        ...
        ax.set_xticklabels(classes, rotation=45, ha="right", color="white")
        ax.set_ylim(0, 1.15)
        ax.set_title("Precision / Recall / F1 per Class — TRAIN SET", color="white",
                     fontsize=13, fontweight="bold", pad=12)
        ...
        for i, v in enumerate(f1s):
            ax.text(x[i] + w, v + 0.02, f"{v:.2f}", ha="center", fontsize=8, color="white")
        plt.tight_layout()
        plt.savefig(str(save_dir / "train_precision_recall_f1_per_class.png"),
                    dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
        plt.close()

    # Plot 4: Overall TP/FP/FN/TN counts
    ov     = metrics["overall"]
    labels = ["TP\n(correct)", "FP\n(false alarm)", "FN\n(missed)", "TN\n(correct empty)"]
    ...
    ax.set_title("Detection Type Counts — TRAIN SET", color="white",
                 fontsize=13, fontweight="bold", pad=12)
    ax.set_ylabel("Count", color="white")
    ax.tick_params(colors="#8b949e")
    for spine in ax.spines.values(): spine.set_color("#30363d")
    plt.tight_layout()
    plt.savefig(str(save_dir / "train_detection_type_counts.png"),
                dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close()
...
def save_csvs(records, image_records, metrics, out_dir: Path):
    det_csv = out_dir / "train_iou_full_per_detection.csv"
    ...
    img_csv = out_dir / "train_iou_full_per_image.csv"
    ...
    cls_csv = out_dir / "train_iou_full_per_class_metrics.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate YOLO model on TRAIN SET with full TP/FP/FN/TN metrics."
    )
    parser.add_argument("--model", required=True,
                        help="Path to trained YOLO model (.pt)")
    parser.add_argument("--data",  required=True,
                        help="Dataset root dir (with images/train and labels/train)")
    ...
    records, image_records = evaluate_iou_train(
